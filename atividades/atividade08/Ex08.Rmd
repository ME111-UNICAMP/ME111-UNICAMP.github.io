...
A distância média de casa até a UNICAMP para estes alunos é $\bar{Y}=\frac{1}{N}\sum_{i=1}^NY_i=`r round(xxxxxxxx,2)`$. Temos duas maneiras de definir a variância populacional: $S^2=\frac{1}{N-1}\sum_{i=1}^N(Y_i-\bar{Y})^2=`r round(xxxxxxx,2)`$ e $\sigma^2=\frac{1}{N}\sum_{i=1}^N(Y_i-\bar{Y})^2=`r round(xxxxxxx,2)`$.
...
Simulacao <- function(ID,Y,n,reposicao="nao"){
  if (reposicao=="nao")
    {
    idAmostrado <- sample(x=XXX,size=XXX,replace=XXXX)
    }
  else
    {
    idAmostrado <- sample(x=XXX,size=XXX,replace=XXX)
    }
    YAmostrado <- XXXXXX # respostas da amostra
    Media <- XXXXXX # média das respostas da amostra
...
```{r}
Simulacao(ID=Populacao$id,Y=Populacao$Y,n=20,reposicao="sim")
```
...
```{r,echo=FALSE,message=FALSE}
renderPlot(hist(DistribuicaoMediaAmostral(ID=Populacao$id,Y=Populacao$Y,n=as.numeric(input$n),reposicao=input$reposicao,B=as.numeric(input$B)),main="Distribuição da Média Amostral",ylab="Frequência", xlab="Média Amostral",col="lightpink"))
options=list(
...
Resultados <- apply(casos,1,function(x) list(DistribuicaoMediaAmostral(ID=Populacao$id,Y=Populacao$Y,n=x[1],reposicao=x[2],B=3000)))
...
S2 <- XXXXXX # $S^2$
sigma2 <- XXXXXXX # $\sigma^2$

BB$VarianciaAmostralTeorica <- ifelse(BB$reposicao=="sim",round(xxxxxxx,2),round(xxxxxx,2))
